...
```bash
vitra new myapp
# or: vitra new myapp --template vite
cd myapp
...
vitra doctor
vitra new ./myapp
# vitra new ./myapp --template vite
vitra dev
vitra build
vitra package --out dist/ [--format dir|deb|rpm-dir|rpm|snap-dir|snap|appdir|appimage|win-dir|wix|nsis-dir|msi|nsis|app-dir|dmg] [--icon path] [--maintainer name] [--description text] [--sign --signing-identity ref]
...
| Distribution (Phase 4) | Specs + Linux stage/`.deb`/`.rpm`/`.snap`/AppDir/AppImage + Windows `win-dir`/`wix`/`nsis-dir`/`msi`/`nsis` + Darwin `app-dir`/`dmg` + signed update apply |
