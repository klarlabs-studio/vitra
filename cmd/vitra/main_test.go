...
func TestRun_NewScaffold(t *testing.T) {
	...
	for _, want := range []string{"createClient", "demo.greet", "dialog.open", "dialog.openDirectory", "dialog.message", "fs.read", "clipboard.read", "browser.open", "os.info", "notifications.show", "path.open", "window.create", "window.close", "window.chrome", "window.getChrome", "window.focus", "window.hide", "window.show", "menu.set", "menu.clear", "tray.set", "tray.clear", "dragdrop.receive", "shortcut.register", "shortcut.unregister", "app.quit", "onMenuAction", "onTrayAction", "onDragdropDrop", "onDeeplinkOpen", "onShortcutAction", "createEvents"} {
