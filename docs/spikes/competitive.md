...
presets beyond Vitra’s `vanilla`/`vite`/`react`/`svelte`/`vue`/`solid`/`preact`/`lit`/`alpine`/`htmx`/`angular`/`qwik`/`mithril`
...
intentionally unsupported. `vitra new --template vite|react|svelte|vue|solid|preact|lit|alpine|htmx|angular|qwik|mithril`
