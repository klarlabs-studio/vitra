...
scaffolds beyond Vitra’s `vanilla`/`vite`/`react` starters; turnkey
...
`vitra new --template vite|react` scaffolds Vite(+React) frontends; `vitra
...
prints a dry-run plan (`PlanSign`) — no codesign/signtool/notary orchestration.
