...
`window.create`/`window.close`/`window.chrome`/`window.getChrome`/`window.focus`/`window.blur`/`window.hide`/`window.show`/`window.minimize`/`window.maximize`/`window.unmaximize`/`window.fullscreen`/`window.unfullscreen`/`window.setAlwaysOnTop`/`window.restore`/`window.setTitle`/`window.setSize`/`window.setIcon` to `desktop.WindowService`
