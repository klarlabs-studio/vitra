...
`window.create`/`window.close`/`window.chrome`/`window.getChrome`/`window.focus`/`window.blur`/`window.hide`/`window.show`/`window.minimize`/`window.maximize`/`window.fullscreen`/`window.setAlwaysOnTop`/`window.restore` to `desktop.WindowService`
