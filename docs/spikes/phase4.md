...
- Wails-class template breadth beyond Vitra’s `vanilla` / `vite` / `react` /
  `svelte` / `vue` / `solid` / `preact` / `lit` / `alpine` / `htmx` / `angular` / `qwik` / `mithril` / `riot` / `inferno` / `stencil` / `marko` / `ember` / `aurelia` / `stimulus` starters (additional frameworks, richer presets)
