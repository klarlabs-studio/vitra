...
| AppStream metainfo | `AppStreamMetainfoXML` staged under `usr/share/metainfo/` (Flatpak `files/share/metainfo/`); Homepage / Categories / License via Spec |
...
- Wails-class template breadth beyond Vitra’s `vanilla` / `vite` / `react` /
  `svelte` / `vue` / `solid` / `preact` / `lit` / `alpine` / `htmx` starters (additional frameworks, richer presets)
