...
| AppStream metainfo | `AppStreamMetainfoXML` staged under `usr/share/metainfo/` (Flatpak `files/share/metainfo/`); Homepage / Categories / License / Description / Maintainer (`developer_name` + `update_contact`) / OARS `content_rating` / `recommends` keyboard+pointing / `releases` from Version / `provides` binary from Name via Spec (also RPM/snap, Debian `copyright`, Windows ARP URL/Comments/LegalCopyright, Darwin `NSHumanReadableCopyright`) |
