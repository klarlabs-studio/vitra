...
| `app-dir` | Staged Darwin `.app` bundle (`StageDarwinApp`; Info.plist + `NSHumanReadableCopyright` + `LSApplicationCategoryType` + `CFBundleGetInfoString` + `CFBundleDisplayName` + `NSHighResolutionCapable` + `NSSupportsAutomaticGraphicsSwitching`) |
