...
// The script installs under $LOCALAPPDATA, writes Uninstall.exe, and registers
// Add/Remove Programs keys under HKCU. Producing a final setup .exe still
// requires makensis (or VITRA_MAKENSIS); this layout is the portable intermediate
// that tool consumes.
func BuildNSISDir(spec Spec, binaryPath, outDir string) (Artifact, error) {
	...
	iconDefine := ""
	iconFileLine := ""
	iconShortcut := `  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}.lnk" "$INSTDIR\${PRODUCT_EXE}"`
	iconUninstall := ""
	displayIconReg := ""
	if spec.IconPath != "" {
		...
		iconShortcut = `  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}.lnk" "$INSTDIR\${PRODUCT_EXE}" "" "$INSTDIR\${PRODUCT_ICON}" 0`
...
`, nsisEscape(spec.Name), nsisEscape(spec.Version), nsisEscape(spec.Name),
		nsisEscape(spec.AppID), exeName, iconDefine, outInstaller, iconFileLine, iconShortcut+"\n",
