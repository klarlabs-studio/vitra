...
func BuildNSISDir(spec Spec, binaryPath, outDir string) (Artifact, error) {
	...
	estimatedSizeReg := fmt.Sprintf("  WriteRegDWORD HKCU \"${UNINST_KEY}\" \"EstimatedSize\" %d\n", windowsARPEstimatedSizeKB(binaryPath, spec.IconPath))
...
`, nsisEscape(spec.Name), nsisEscape(spec.Version), nsisEscape(spec.EffectivePublisher()),
		...
		displayIconReg, homepageReg, commentsReg, copyrightReg, estimatedSizeReg, iconUninstall)
