...
func TestTrySingleInstance_Exclusive(t *testing.T) {
	h := New()
	held, release, err := h.TrySingleInstance("vitra-windows-si-test")
	if err != nil || !held {
	...
	h2 := New()
	held2, release2, err := h2.TrySingleInstance("vitra-windows-si-test")
	if err != nil {
...
func TestDeepLinkBridge_Handoff(t *testing.T) {
	const appID = "vitra-windows-dl-test"
	h := New()
...
func TestForwardToPrimary_NoListener(t *testing.T) {
	h := New()
	ok, err := h.ForwardToPrimary("vitra-windows-dl-missing", []string{"vitra://x"})
	if err != nil {
		t.Fatal(err)
	}
	if ok {
