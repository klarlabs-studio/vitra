...
func TestOfficialPlugins_RegisterCleanly(t *testing.T) {
	...
	for _, p := range []plugin.Plugin{
		...
		officialos.New(), notification.New(), officialpath.New(), officialwindow.New(), menu.New(), tray.New(),
	} {
	...
	for _, id := range []domain.PluginID{
		...
		officialos.PluginID, notification.PluginID, officialpath.PluginID, officialwindow.PluginID, menu.PluginID, tray.PluginID,
	} {
