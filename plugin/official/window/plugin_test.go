...
func TestWindowPlugin_Contribute(t *testing.T) {
	...
	want := []string{
		...
		"window.minimize", "window.maximize", "window.fullscreen", "window.setAlwaysOnTop",
	}
	...
	for i, name := range []string{"getChrome", "focus", "blur", "hide", "show", "minimize", "maximize", "fullscreen", "setAlwaysOnTop"} {
		idx := i + 3
